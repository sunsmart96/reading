...
# 4. 通信协议
RPC 的底层通信通常使用 socket，但具体协议可以自定义。传输层常见协议有 TCP 和 UDP，应用层常见协议有 HTTP，也可以根据业务需求设计自定义协议。

## Dubbo 协议示意图

![Dubbo 协议帧结构示意图](assets/dubbo-protocol-frame.svg)

这个图是 AI 生成后再根据 Dubbo 协议资料调整的。我在很多项目中设计和实现过私有协议，基本思路就是先定义固定字段，再做扩展和解析，底层处理的都是二进制位或者字节。如果是通用一点的协议，还要考虑兼容性之类的问题。实现时封装好解析器和写入器即可；如果想快速解析，还可以在里面使用 SIMD 之类的方法。

有了底层 socket 的通信能力支持，还需要自定义协议来解析数据。这就是这部分内容存在的目的。

# 5. 序列化
序列化就是把程序中的对象转换为可存储或者可传输的数据形式。反序列化就是把网络传输过来的数据或者磁盘存储的数据转换为编程语言中的对象。文本数据形式比较常用的有 JSON、XML，二进制序列化通常编码更紧凑，传输效率也更高一些。我常用的序列化工具主要是 Protobuf，最近在研究 DDS，也会用 IDL，或者 ROS 中的 msg，基本原理都差不多，不过 Google 的产品性能会好一些。

![序列化与反序列化示意图](assets/serialization.svg)

# 6. 动态代理
在项目中使用 RPC 框架的初衷，就是快速进行跨机器通信，把远程函数调用做得像本地函数调用一样方便。除了客户端和服务端，本地和远程的存根实际上就是充当代理的角色，用来屏蔽底层的通信、序列化、校验等过程，让上层应用专注于业务开发。书里面介绍了一些 Java 实现动态代理的方法，基本上需要用到反射。这几个框架的细节我没研究过，就不过多评论了。

# 7-8. 实现一个简易的 RPC 框架/异构语言的调用
通过调用现成的库做一个 demo 其实不难，再不济也就是自己在 socket 上做各种协议和数据解析。作者用 Java 写了示例。第八章讲跨语言调用，这块主要涉及语言适配和编译器。做得比较好的就是 gRPC。

# 9-14. 服务治理
这部分其实是大规模云端分布式后端架构体系的简略版，不完全是 RPC 本身的内容。比如注册中心有利于服务迁移和扩展。分布式系统肯定要考虑分布式一致性，比如 Raft 算法。注册中心的方案有 Eureka、ZooKeeper、Nacos 等。对于大型分布式后端服务体系，不同服务会有不同配置，而且配置可能会变更，甚至需要重启，所以也需要配置中心。对于复杂服务，仅仅有配置文件还不够，还需要元数据。元数据在数据存储和传输中也非常常见，因此也可以配置元数据中心。为了做流量调峰，还可以设计一些路由策略；为了提高系统可观测性，也可以建设分布式日志系统等。这些都不是绝对的，要根据实际需求进行设计。
