...
这个图其实是 AI 生成的，我也不确定准不准。不过我在很多项目中设计实现过私有协议，基本思路就是先定义固定的字段，然后再去做拓展和解析，都是二进制位或者字节的。如果是通用一点的协议还要考虑兼容性之类的问题。写解析器写入器之类的封装就行，如果想快速解析，还可以在里面用一些 SIMD 之类的方法。

有了底层 socket 的通信能力支持，就需要自定义协议来解析数据。这就是这部分存在的目的。
